...
class AreaChart extends Component {
  ...
  setChartData = (json) => {
    const chartData = []
    json.map((data, index)=> (
      chartData.push([new Date(data.date).getTime(),data.close])
    ))
    this.setState({
      series:[{data: chartData, name: this.state.tickerToLook}],
      options: {
        xaxis: {
          min: new Date(chartData[1][0]).getTime(),
          max: new Date(chartData.slice(-1).pop()[0]).getTime(),
        }
      }
    })
  }
  ...
  //   switch (timeline) {
  //     case '1m':
  //       this.getChartData(this.state.tickerToLook,"1m")
  //       break;
  //     case '3m':
  //       this.getChartData(this.state.tickerToLook,"3m")
  //       break;
  //     case '6m':
  //       this.getChartData(this.state.tickerToLook,"6m")    
  //       break;
  //     case '1y':
  //       this.getChartData(this.state.tickerToLook,"1y")
  //       break;
  //     case 'ytd':
  //       this.getChartData(this.state.tickerToLook,"ytd")
  //       break;
  //     case '2y':
  //       this.getChartData(this.state.tickerToLook,"2y")
  //       break;
  //     case '5y':
  //       this.getChartData(this.state.tickerToLook,"5y")
  //       break;
  //     default:
  //   }
      ...
      <Fragment>
        <div id="chart">
          <div className="toolbar">
            ...
            <button onClick={()=>this.updateData('3m')} id="3m" className={ (this.state.selection==='3m' ? 'active' : '')}>3M</button>
            <button onClick={()=>this.updateData('6m')} id="6m" className={ (this.state.selection==='6m' ? 'active' : '')}>6M</button>
            <button onClick={()=>this.updateData('1m'/*disabled*/)} id="1y" className={ (this.state.selection==='1y' ? 'active' : '')}>1Y</button>
            <button onClick={()=>this.updateData('1m'/*disabled*/)} id="ytd" className={ (this.state.selection==='ytd' ? 'active' : '')}>YTD</button>
            <button onClick={()=>this.updateData('1m'/*disabled*/)} id="2y" className={ (this.state.selection==='2y' ? 'active' : '')}>2Y</button>
            <button onClick={()=>this.updateData('1m'/*disabled*/)} id="5y" className={ (this.state.selection==='5y' ? 'active' : '')}>5Y</button>
          </div>
